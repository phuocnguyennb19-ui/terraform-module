# LAMBDA FUNCTIONS
#
# VPC-attached functions consume the foundation's private subnets and the
# platform's lambda security group. This module creates neither.
#
# Upstream: terraform-aws-modules/lambda/aws v7, which handles packaging, the
# execution role, the log group and the VPC ENI permissions as one unit.
...
module "function" {
  ...
  # The execution role needs ec2:CreateNetworkInterface to attach to the VPC.
  # Granting it unconditionally would give every function permissions it does
  # not use, so it follows vpc_attached.
